...
public class MyArrayList<T> implements List<T> {
    ...
    public T get(int index) {
        validateIndexBeforeOperate(index, size);
        return array[index];
    }
    ...
    /**
     * Validate the index before operating with the array.
     * If anything is wrong, throw an {@link IndexOutOfBoundsException}.
     *
     * @param index the index to operate
     * @param size  the array size
     */
    private void validateIndexBeforeOperate(final int index, final int size) {
        if (index < 0 || index >= size) {
            throw new IndexOutOfBoundsException();
        }
    }
